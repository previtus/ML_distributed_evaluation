...
def _main_modelless(args, yolo_model, sess, direct_images, crops_bboxes, crop_value, resize_frames=None, verbose=1, person_only=True, allowed_number_of_boxes=100):
    '''
    ...
    for image_i in range(0,len(direct_images)):
        ...
        frame = direct_images[image_i]

        if resize_frames is not None:
            resize_to = resize_frames[image_i]
        ...
        crops_in_frame = crops_bboxes[image_i]
        #print("Frame", frame_i, " with ", len(crops_in_frame), " crops.")
        sys.stdout.write("\rFrame " + str(image_i) + " with " + str(len(crops_in_frame)) + " crops.")
        sys.stdout.flush()
        ...
        for crop_i in range(0, len(crops_in_frame)):
            ...
            if verbose > 0:
                ...
                if person_only:
                    num = people
                print('Found {} boxes in crop {} of frame {} in {}s'.format(num, crop_i, image_i, evaluation_time))
